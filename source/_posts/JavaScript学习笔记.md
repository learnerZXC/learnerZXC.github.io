...
数值转换： 将其他类型转换为字符串有两种方法：
 - toString()
    ...
    参数：str是要转换的类型，可以是数值、布尔型、对象和字符串。
...
2. 如slice()方法吃参数中有一个负数，则用数组长度加上该数来确定相应的位置。
...
3. 如slice()方法吃参数中有一个负数，则用数组长度加上该数来确定相应的位置。
...
2. substring()会将较小的数作为开始位置，把较大的数作为结束位置。
...
语法：`location.href`或`location.href='URL'`
